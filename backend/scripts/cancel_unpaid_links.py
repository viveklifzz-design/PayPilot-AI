...
res_pl = requests.get("https://api.razorpay.com/v1/payment_links", auth=(key_id, key_secret))
if res_pl.status_code == 200:
    plinks = res_pl.json().get("payment_links", [])
    cancelled = 0
    for l in plinks:
        if l.get("status") == "created" and l.get("amount") == 250000:
            pid = l["id"]
            res = requests.post(f"https://api.razorpay.com/v1/payment_links/{pid}/cancel", auth=(key_id, key_secret))
            if res.status_code == 200:
                cancelled += 1
                print(f"Cancelled Payment Link: {pid}")
    print(f"Cancelled total {cancelled} old unpaid 2500 links.")
